...
def render_holdings_section(names, show_allocation_chart: bool = True) -> None:
    ...
    if len(names) > 1 and show_allocation_chart:
...
def _pdf_section_table(pdf, section_title, names, sim_results, profile_kwargs, asset_df, cpi,
    ...
    for name in ordered_names(names):
        ...
        irr_shown = irr_pot if show_ruin else irr
        ...
        pdf.cell(col_widths[col], 8, f"{irr_shown*100:.2f}%" if not np.isnan(irr_shown) else "n/a", border=1); col += 1
...
def build_summary_pdf(accum_results: dict, decum_results: dict, accum_profile_kwargs: dict,
    ...
    pdf.multi_cell(
        ...
        "alone, excluding State Pension - a flat, guaranteed amount identical regardless of which "
        "portfolio is held, so excluding it is the fairer basis for comparing portfolios against "
        "each other.",
    )
